    ...
    <div className="bg-gray-100 fixed mt-[-20px] z-10 w-full">
      ...
      <nav className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 relative">
          <div className="flex items-center justify-between lg:justify-center lg:space-x-5 h-12">
            {/* Hamburger menu button for mobile */}
            <button
              onClick={() => setIsMenuOpen(!isMenuOpen)}
              className="lg:hidden text-gray-700 hover:text-black p-1"
            >
              {isMenuOpen ? (
                <X className="w-5 h-5" />
              ) : (
                ...
                {navbarItems.map((option) => {
                  return (
                    <a
                      key={option.id}
                      href={option.path}
                      className="block text-gray-700 text-sm hover:text-black py-2"
