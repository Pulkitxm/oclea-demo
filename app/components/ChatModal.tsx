...
const ChatModal: React.FC<ChatModalProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    function handle(e: MouseEvent) {
      // Don't close if clicking on the chat button itself
      const target = e.target as Element;
      const chatBtn = target.closest("[data-chat-btn]");
      if (chatBtn) return;
      ...
      >
        ...
        >
          <ul className="flex flex-col items-stretch gap-3 m-0 p-0 list-none px-2 py-2">
            ...
            >
              <span role="img" aria-label="Copy in LLM">
                📋
              </span>{" "}
              Copy for LLM
              {copied && (
                <span className="absolute -top-6 left-1/2 -translate-x-1/2 bg-green-200 text-green-800 px-2 py-0.5 rounded text-xs shadow">
                  Copied!
                </span>
              )}
            </li>
            ...
            >
              <span role="img" aria-label="Open in ChatGPT">
                🌐
              </span>{" "}
              Open in ChatGPT
            </li>
            ...
            >
              <span role="img" aria-label="Open in Claude">
                ☁️
              </span>{" "}
              Open in Claude
            </li>
